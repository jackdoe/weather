...
const generateStats = require('./parse');

async function a() {
  const australianApiStats = await generateStats
  
  let data = [];
  let errorCount = 0;
  australianApiStats.forEach(city => {
    ...
    try {
      ...
      // if (response.status >= 400) getWeatherFromMetno();
      ...
      parseString(xmlData, (err, result) => {
        ...
        data.push({
          ...
          weather: {
            ...
            fogPercent: Number(getDataFrom(fog).percent),
